...
MAPFILES='../../human_genome_data/hapmap2/genetic_map_%s_b36.txt'
...
def runSVM(fileNames, nGens=6, svmC=100, win_size=100):
    ...
    filter=regionClassifier.hmmFilter(geneticMapFile=MAPFILES%chrom,nGens=nGens,nClasses=len(fileNames)-1)
...
if __name__ == '__main__':
    ...
    parser.add_option("-g", "--generations", type='int', dest="nGens", default=6,
                      help="Number of generations sinces admixture used in hmm. (default 6)", metavar="N")
    (options, args) = parser.parse_args()

    fileNames=args
    snpLocations, ancestralSuccess, admixedClassPre, admixedClass = runSVM(fileNames, nGens=options.nGens, win_size=options.win)
    ...
    pylab.ylabel('Sample ');pylab.yticks([]); pylab.xticks([]); pylab.axis('tight'); 
    pylab.title('Position along %s' %chrom)
    ...
    if options.correctFile:
        ...
        print 'Correct %0.3g +/- %0.2g (%0.3g +/- %0.2g)' %(np.mean(svmSuccess), np.std(svmSuccess), 
                                                            np.mean(hmmSuccess), np.std(hmmSuccess))
        pylab.figure();
        pylab.subplot(2,1,1);
